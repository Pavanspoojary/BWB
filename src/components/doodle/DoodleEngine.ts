...
void main() {
  ...
  float rule = (1.0 - smoothstep(0.5 * sc, 1.7 * sc, abs(ly - ls * 0.5))) * uPaperLines;
  paper = mix(paper, vec3(0.58, 0.70, 0.92), rule * 0.45);

  float margin = (1.0 - smoothstep(0.9 * sc, 2.3 * sc, abs(pp.x - uRes.x * 0.07))) * uPaperLines;
...
export class DoodleEngine {
  ...
  private setupSkyAndEnvironment() {
    ...
    // NOTE: blank-slate rebuild — sun, clouds and paper planes all removed.
    // Sky decor comes back piece by piece with the world below it.
